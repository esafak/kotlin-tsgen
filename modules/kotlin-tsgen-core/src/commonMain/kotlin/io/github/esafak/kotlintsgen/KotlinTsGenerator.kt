...
private val builtInPrimitiveSerialNames =
  setOf(
    "kotlin.Boolean",
    "kotlin.Byte",
    "kotlin.Char",
...
open class KotlinTsGenerator(
  ...
  private val descriptorsCache: MutableMap<KSerializer<*>, Set<SerialDescriptor>> = mutableMapOf()
  private val elementIdCache: MutableMap<SerialDescriptor, TsElementId> = mutableMapOf()
  private val mapTypeCache:
    MutableMap<Pair<SerialDescriptor, SerialDescriptor>, TsLiteral.TsMap.Type> =
    mutableMapOf()
  private val typeRefCache: MutableMap<SerialDescriptor, TsTypeRef> = mutableMapOf()
  private val elementCache: MutableMap<SerialDescriptor, Set<TsElement>> = mutableMapOf()
  ...
  private fun findOverride(descriptor: SerialDescriptor): TsElement? =
    descriptorOverrides.entries
      .run {
        firstOrNull { it.key == descriptor }
          ?: firstOrNull { it.key.nullable == descriptor.nullable }
      }?.value

  open fun findMapTypeOverride(descriptor: SerialDescriptor): TsLiteral.TsMap.Type? =
    when (findOverride(descriptor)) {
      null -> null
  ...
  open val descriptorsExtractor =
    object : SerializerDescriptorsExtractor {
  ...
  val elementIdConverter: TsElementIdConverter =
    object : TsElementIdConverter {
      ...
      override fun invoke(descriptor: SerialDescriptor): TsElementId =
        elementIdCache.getOrPut(descriptor) {
  ...
  val mapTypeConverter: TsMapTypeConverter =
    object : TsMapTypeConverter {
      ...
      override fun invoke(
        ...
        mapTypeCache.getOrPut(keyDescriptor to valDescriptor) {
          findMapTypeOverride(keyDescriptor) ?: converter(keyDescriptor, valDescriptor)
        }
    }

  val typeRefConverter: TsTypeRefConverter =
    object : TsTypeRefConverter {
      private val converter =
        object : TsTypeRefConverter.Default(
          elementIdConverter,
          mapTypeConverter,
          effectiveSerializersModule,
          primitiveTypeRefOverride = { descriptor ->
            generatedPrimitiveAliases[normalizedSerialName(descriptor.serialName)]
          ...
          indexSignatureKeyTypeRefOverride = { descriptor ->
            when (val override = findOverride(descriptor)) {
              ...
              TsLiteral.Primitive.TsNumber,
              TsLiteral.Primitive.TsString,
              -> TsTypeRef.Literal(override as TsLiteral, false)

              else -> throw InvalidTsIdentifierException(
                descriptor.serialName,
                "index signature key",
                "only primitive string and number keys are supported",
              )
            }
          },
        ) {
          override fun mapKeyTypeRefOverride(descriptor: SerialDescriptor): TsTypeRef? =
            (findOverride(descriptor) as? TsLiteral)?.let {
              TsTypeRef.Literal(it, descriptor.isNullable)
            }
        }

      override fun invoke(descriptor: SerialDescriptor): TsTypeRef =
        typeRefCache.getOrPut(descriptor) {
          when (val override = findOverride(descriptor)) {
  ...
  val elementConverter: TsElementConverter =
    object : TsElementConverter {
      ...
      override fun invoke(descriptor: SerialDescriptor): Set<TsElement> =
        elementCache.getOrPut(descriptor) {
          when (val override = findOverride(descriptor)) {
            null -> converter(descriptor)
            else -> setOf(override)
          }
  ...
  private fun primitiveAlias(descriptor: SerialDescriptor): TsDeclaration.TsTypeAlias? {
    val primitiveKind = descriptor.kind as? PrimitiveKind ?: return null
    if (normalizedSerialName(descriptor.serialName) in builtInPrimitiveSerialNames &&
      primitiveKind !in signedPrimitiveKinds
    ) {
      return null
    }
    if (findOverride(descriptor) != null) return null

    val id = generatedPrimitiveAliases[normalizedSerialName(descriptor.serialName)] ?: return null
  ...
  private val signedPrimitiveKinds =
    setOf(
      PrimitiveKind.BYTE,
      PrimitiveKind.SHORT,
      PrimitiveKind.INT,
  ...
  private fun normalizedSerialName(serialName: String): String = serialName.removeSuffix("?")
  ...
  open fun generate(vararg serializers: KSerializer<*>): String {
      ...
      descriptors
        .filter { descriptor ->
          ...
          kind !in setOf(PrimitiveKind.BOOLEAN, PrimitiveKind.CHAR, PrimitiveKind.STRING) ||
            normalizedSerialName(descriptor.serialName) !in builtInPrimitiveSerialNames
        }.filter { findOverride(it) == null }
        .distinctBy { it.serialName.removeSuffix("?") to it.kind }
    val aliasEntries =
      primitiveAliasDescriptors.map { descriptor ->
        val serialName = normalizedSerialName(descriptor.serialName)
        val id =
          if (serialName in builtInPrimitiveSerialNames) {
    ...
    aliasEntries
      .groupingBy { (_, id) ->
        sourceCodeGenerator.groupElementsBy(
          TsDeclaration.TsTypeAlias(id, TsTypeRef.Literal(TsLiteral.Primitive.TsString, false)),
        ) to
          id.name
      }.eachCount()
      .filterValues { it > 1 }
      ...
      ?.let { (_, name) ->
        throw InvalidTsIdentifierException(
          name,
          "generated numeric type aliases",
          "multiple reachable primitive aliases render with the same name",
        )
      }
    ...
    aliases
      .filter { alias ->
        (sourceCodeGenerator.groupElementsBy(alias) to alias.id.name) in
          declarationGroups
      }.firstOrNull()
      ?.let { alias ->
        val conflict =
  ...
  private fun canMergeDeclarations(declarations: List<TsDeclaration>): Boolean =
    (
      declarations.all { it is TsDeclaration.TsInterface } &&
        declarations
